...
class FTDClient:
    ...
    def setup_logging(self):
        """
        Setup a simple logging for debug
        """
        tempdir = tempfile.gettempdir()
        logfile = os.path.join(tempdir, 'python_test_client.log')
        logging.basicConfig(filename=logfile, level=logging.DEBUG, format='%(asctime)s %(levelname)-8s %(message)s', datefmt='%m-%d %H:%M:%S')
    ...
    def login(self):
        ...
        try:
            self.access_token = r.json()['access_token']    # This token will act as the 
            self.original_access_token = self.access_token  # cache the original token in case we do a custom login
        except:
    ...
    def __exit__(self, exception_type, exception_value, exception_traceback):
        ...
        if exception_type:
            logging.error("ERROR: {} ({}) -- {}".format(exception_type, exception_value, exception_traceback))
        else:
            self.logout()
